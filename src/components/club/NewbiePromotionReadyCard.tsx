...
import { promoteToRookie } from "@/app/(app)/promotions-review/actions";

export type NewbiePromotionReadyMember = {
  userId: string;
  name: string;
  avatarUrl: string | null;
  approvedCount: number;
  requiredCount: number;
  verifiedEquipmentCount: number;
  requiredEquipmentCount: number;
  introToRokDone: boolean;
};

/** Mirrors PromotionReadyCard.tsx's structure — same Avatar + stats line +
 * finalize button + inline message pattern — but this stage has no exam:
 * must-skills + equipment verification are the whole bar, not R1/R2. */
export function NewbiePromotionReadyCard({ member }: { member: NewbiePromotionReadyMember }) {
  ...
  function handleFinalize() {
    ...
    startTransition(async () => {
      const result = await promoteToRookie(member.userId);
      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="flex min-w-0 flex-col gap-0.5">
          ...
          <span className="text-xs text-charcoal-light/70">
            {member.approvedCount}/{member.requiredCount} Drives · Must Skills ✓ · Equipment{" "}
            {member.verifiedEquipmentCount}/{member.requiredEquipmentCount} Verified
          </span>
        </div>
      </div>

      {member.introToRokDone ? (
